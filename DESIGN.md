...
> **溯源说明（实现后补记）**：本文是开工前的原始草案，保留原样以便对照设计意图与最终实现的差异。
> 两处已知出入：
> 1. 技术栈由 **Next.js 全栈单体** 改为 **FastAPI + React SPA（同镜像托管）**——按「后端必须用 Python FastAPI」的要求调整；§6 的验收清单、§5 的分段部署思路、迁移独立成步等设计意图均已落实。
> 2. 本文写作时**代码未开工**；当前仓库已完整实现并跑通管线，实际结果与证据见 `report/REPORT.md`。
>
> 目的：**验证一条完整的 CI/CD 管线**。业务故意做到最简，评审时注意力应全部落在管线上。
> 状态：草案，代码未开工（**已过时，见上方溯源说明**）。
