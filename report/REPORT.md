...
| `latest.yml`（更新源） | 改由**应用自己**在 `/updates/latest.yml` 提供——新 job `update-feed` 负责发布并打公网验证。下载页因此只剩一个文件，客户端要的东西一样没少 |
| `*.exe.blockmap` | 同样搬到应用上（**必须与安装包同源**：客户端把块文件地址当成"安装包地址 + `.blockmap`"），所以**增量更新保留**——常规更新只下载变化的块 |

安装包本身**不搬到服务器**：实测 CI runner(美国) → 服务器只有约 28 KB/s，110 MB 要传一小时。
应用对 `/updates/<安装包名>` 返回 **302 跳到 GitHub 的 Release 附件**，客户端带着 Range 请求
跟着跳转——字节从 GitHub 取（它们的带宽），块文件从我们这里取（同源），增量成立。
服务器上只存 feed 与块文件（几百 KB）。
